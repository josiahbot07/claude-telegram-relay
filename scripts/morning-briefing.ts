...
function buildPrompt(summaries: string[], quote: Quote): string {
  ...
  return [
    ...
    "- One fun/interesting thing about today (a historical event, holiday, or fun fact)",
    `- This quote from ${quote.conference} General Conference — incorporate it naturally with the speaker's name:`,
    `  "${quote.quote}" — ${quote.speaker}, "${quote.talk}"`,
